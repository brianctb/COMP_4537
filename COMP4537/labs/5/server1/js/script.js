...
class Client {
    ...
    displayRows(rows) {
        this.textAreaResults.style.color = "black";
        let table = "<table border='1'><tr><th>Patient ID</th><th>Name</th><th>Date of Birth</th></tr>";
        rows.forEach(row => {
            table += `<tr>
                        <td>${row.patientid}</td>
                        <td>${row.name}</td>
    ...
    insertDefaultData() {
        ...
        xhttp.open("POST", `https://comp-4537-1-xeb8.onrender.com/api/v1/sql`, true);
    ...
    selectData(query) {
        const xhttp = new XMLHttpRequest();
        xhttp.open("GET", `https://comp-4537-1-xeb8.onrender.com/api/v1/sql/?query=${query}`, true);
        xhttp.send();
        xhttp.onreadystatechange = () => {
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                let response = JSON.parse(xhttp.responseText);
